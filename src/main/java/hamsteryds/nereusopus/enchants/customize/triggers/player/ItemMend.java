...
public class ItemMend extends Trigger {
    public ItemMend() {
        super(ActionType.ITEM_MEND);
    }

    @Override
    public Pair<ParamType, Object> get(Event evt, String name) {
        PlayerItemMendEvent event = (PlayerItemMendEvent) evt;
        ParamType type = null;
        Object object = null;
        ...
        if (name.equals("repairAmount")) {
            type = ParamType.INTEGER;
            object = event.getRepairAmount();
        }
    ...
    public void set(Event evt, String name, Object value) {
        PlayerItemMendEvent event = (PlayerItemMendEvent) evt;
        if (name.equals("repairAmount")) {
            event.setRepairAmount((Integer) value);
        }
        if (name.equals("cancel")) {
