...
public class SwapHandItems extends Trigger {
    public SwapHandItems() {
        super(ActionType.SWAP_HAND_ITEMS);
    }

    @Override
    public Pair<ParamType, Object> get(Event evt, String name) {
        PlayerSwapHandItemsEvent event = (PlayerSwapHandItemsEvent) evt;
        ParamType type = null;
        Object object = null;
        ...
        if (name.equals("mainHandItem")) {
            type = ParamType.ITEM;
            object = event.getMainHandItem();
        }
        if (name.equals("offHandItem")) {
            type = ParamType.ITEM;
            object = event.getOffHandItem();
        }
    ...
    public void set(Event evt, String name, Object value) {
        PlayerSwapHandItemsEvent event = (PlayerSwapHandItemsEvent) evt;
        if (name.equals("mainHandItem")) {
            event.setMainHandItem((ItemStack) value);
        }
        if (name.equals("offHandItem")) {
            event.setOffHandItem((ItemStack) value);
        }
        if (name.equals("cancel")) {
